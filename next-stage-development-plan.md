...
- `design/index.html`：FluxGate MVP-01 手机端 + 平板端重新设计原型。
...
下一阶段目标不是继续扩展 POC，而是把已经验证和部分实现的能力收敛成一个可日常试用的 FluxGate MVP-02：
...
- `Index.ets` 已迁移为 FluxGate 深色 UI 基线，首屏为运行仪表盘。
