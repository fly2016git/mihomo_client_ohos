...
func (h *ListenerHandler) NewConnection(ctx context.Context, conn net.Conn, metadata M.Metadata) error {
	if h.ShouldHijackDns(metadata.Destination.AddrPort()) {
		log.Infoln("[DNS] hijack tcp:%s", metadata.Destination.String())
		err := resolver.RelayDnsConn(ctx, conn, resolver.DefaultDnsReadTimeout)
		if err != nil {
			log.Warnln("[DNS] relay tcp failed for %s: %s", metadata.Destination.String(), err)
		}
		return err
	}
...
func (h *ListenerHandler) NewPacket(ctx context.Context, key netip.AddrPort, buffer *buf.Buffer, metadata M.Metadata, init func(natConn network.PacketConn) network.PacketWriter) {
	if h.ShouldHijackDns(metadata.Destination.AddrPort()) {
		log.Infoln("[DNS] hijack udp:%s from %s", metadata.Destination.String(), metadata.Source.String())
		writer := init(nil)
...
func relayDnsPacket(ctx context.Context, readBuff *buf.Buffer, rwOptions network.ReadWaitOptions, dest M.Socksaddr, mutex *sync.Mutex, writer *network.PacketWriter) {
	...
	if err != nil {
		log.Warnln("[DNS] relay udp failed for %s: %s", dest.String(), err)
		writeBuff.Release()
		return
	}
	...
	conn := *writer
	if conn == nil {
		log.Warnln("[DNS] relay udp dropped for %s: writer closed", dest.String())
		writeBuff.Release()
		return
	}
	err = conn.WritePacket(writeBuff, dest) // WritePacket will release writeBuff
	if err != nil {
		log.Warnln("[DNS] relay udp write failed for %s: %s", dest.String(), err)
		writeBuff.Release()
		return
	}
}
