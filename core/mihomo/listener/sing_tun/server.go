...
func New(options LC.Tun, tunnel C.Tunnel, additions ...inbound.Addition) (l *Listener, err error) {
	...
	log.Infoln("[TUN] init fd=%d name=%s stack=%s inet4=%v inet6=%v dnsHijack=%v autoRoute=%v autoDetectInterface=%v",
		options.FileDescriptor, tunName, options.Stack, tunOptions.Inet4Address, tunOptions.Inet6Address,
		options.DNSHijack, options.AutoRoute, options.AutoDetectInterface)
	tunIf, err := tunNew(tunOptions)
	if err != nil {
		log.Warnln("[TUN] tunNew failed fd=%d name=%s: %s", options.FileDescriptor, tunName, err)
		err = E.Cause(err, "configure tun interface")
		return
	}
	log.Infoln("[TUN] tunNew ready fd=%d name=%s", options.FileDescriptor, tunName)
	...
	log.Infoln("[TUN] stack create begin fd=%d stack=%s", options.FileDescriptor, options.Stack)
	tunStack, err := tun.NewStack(strings.ToLower(options.Stack.String()), stackOptions)
	if err != nil {
		log.Warnln("[TUN] stack create failed fd=%d stack=%s: %s", options.FileDescriptor, options.Stack, err)
		return
	}
	log.Infoln("[TUN] stack create ready fd=%d stack=%s", options.FileDescriptor, options.Stack)

	err = tunStack.Start()
	if err != nil {
		log.Warnln("[TUN] stack start failed fd=%d stack=%s: %s", options.FileDescriptor, options.Stack, err)
		return
	}
	log.Infoln("[TUN] stack start ready fd=%d stack=%s", options.FileDescriptor, options.Stack)
	l.tunStack = tunStack
	...
	log.Infoln("[TUN] listener ready %s", l.addrStr)
	return
}
...
func (l *Listener) Close() error {
	...
	if l.options.FileDescriptor == 0 {
		closers = append(closers, l.tunStack)
	} else {
		log.Infoln("[TUN] skip closing fd-backed tun stack fd=%d", l.options.FileDescriptor)
	}
	if l.options.FileDescriptor == 0 {
		closers = append(closers, l.tunIf)
	} else {
		log.Debugln("[TUN] skip closing fd-backed tun interface fd=%d", l.options.FileDescriptor)
	}
